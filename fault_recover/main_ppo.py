...
class FaultRecoverTaskRunner(TaskRunner):
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            ...
            from recipe.fault_recover.megatron_workers import AsyncFaultRecoverActorRolloutRefWorker

            actor_rollout_cls = AsyncFaultRecoverActorRolloutRefWorker
            ray_worker_group_cls = RayWorkerGroup
    ...
    def run(self, config):
        ...
        if config.fault_manager.enable:
            with open_dict(config.actor_rollout_ref):
                config.actor_rollout_ref.fault_manager = config.fault_manager
            FaultMgr.init_tokens_queue()

        # Initialize the PPO trainer.
        trainer = FaultRecoverRayPPOTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
